...
### Supported Languages
- English (with [Cambridge Dictionary](https://dictionary.cambridge.org/)) - **For Russian speakers**

### Features for Russian Speakers (English)
The English card generator includes special features for Russian-speaking learners:
- **Russian translation** - Direct translation of the word/phrase
- **False friends warnings** - Alerts for words like "accurate ≠ аккуратный", "magazine ≠ магазин"
- **Pronunciation hints** - Stress markers and difficult sounds (th, w/v, vowel length)
- **Preposition patterns** - Common differences from Russian ("depend ON" vs "зависеть ОТ")
- **Collocations** - Natural word combinations ("make a decision" NOT "do a decision")

### Card Directions
You can generate different card types based on learning direction:

| Direction | Front | Back | Purpose |
|-----------|-------|------|---------|
| `recognition` | English word + audio | Definition, Russian translation, examples, image | Passive vocabulary - understand English |
| `production` | Russian translation + hint | English word + audio, definition, examples, image | Active vocabulary - produce English |
| `both` | Creates both cards | | Complete vocabulary training |

**Usage examples:**
```bash
# Recognition only (default) - English → Russian
python -m generator.read-generate-import ./input.csv ./processing

# Production only - Russian → English
python -m generator.read-generate-import ./input.csv ./processing --card_direction production

# Both card types - complete training
python -m generator.read-generate-import ./input.csv ./processing --card_direction both
```

**Tip:** Start with `recognition` to build passive vocabulary, then add `production` cards for words you want to use actively.

### Cloze Deletion Cards
Cloze cards help practice collocations, prepositions, and word usage in context.

**Card Structure:**
- **Front:** Sentence with blank + Russian hint: "She {{c1::agreed}} to help us." (согласилась)
- **Back:** Answer revealed + notes

**Usage:**
```bash
# Add cloze cards to any card direction
python -m generator.read-generate-import ./input.csv ./processing --include_cloze

# Full set: recognition + production + cloze
python -m generator.read-generate-import ./input.csv ./processing --card_direction both --include_cloze
```

**Focus areas:**
- Preposition patterns: "interested IN", "depend ON"
- Collocations: "make a decision" (NOT "do a decision")
- Phrasal verbs: "give UP", "put OFF"

**Note:** Cloze cards require the "Cloze" model in Anki (built-in by default).
...
### Synchronization
Cards are imported into local Anki client, and pictures are copied to the Anki media folder.

**Media folder locations by OS:**
- Windows: `C:\Users\User\AppData\Roaming\Anki2\User 1\collection.media`
- macOS: `~/Library/Application Support/Anki2/User 1/collection.media`
- Linux: `~/.local/share/Anki2/User 1/collection.media`

Afterward, Anki can be synchronized with AnkiWeb, and the deck can be used from other devices. Versions for different devices can downloaded [here](https://apps.ankiweb.net/#windows).
...
read-generate-import.py input_file processing_directory \
          ...
          [--card_direction {recognition,production,both}] \
          [--include_cloze] \
          [--image_generation_mode {openai,replicate}] \
          [--replicate_api_key REPLICATE_API_KEY]\
          [--replicate_model_url REPLICATE_MODEL_URL]
```
...
- Language is English
- Level is C1
- OpenAI API key is expected to be set in environment variable OPENAI_API_KEY
- Deck name is autogenerated, containing current date
- Default Anki media directory path for Windows, Linux and macOS
- Card model "Basic" (recommended for proper HTML rendering)
- Card direction is "recognition" (English → Russian)
- Cloze cards are disabled by default
- Image generation mode OpenAI

**Note:** The "Basic (type in the answer)" model strips HTML formatting. Use `--card_model "Basic"` for properly styled cards with images and formatting.
...
python -m generator.read-generate-import ./demo/input_words.csv ./processing  \
          ...
          --replicate_model_url="stability-ai/stable-diffusion:ac732df83cea7fff18b8472768c88ad041fa750ff7682a21affe81863cbe77e4"
```
...
- JSON with structured card data:
  - English definition
  - Russian translation
  - 3 context sentences
  - Notes (collocations, pronunciation, formality)
  - Russian speaker tips (false friends, preposition patterns) - when relevant
- PNG with AI-generated image (via DALL-E 3 or Replicate model)
- MP3 with pronunciation audio (via OpenAI TTS)
- Dictionary link (if found)
...
*****
Q: My cards show plain text without formatting, images, or styling.
A: The "Basic (type in the answer)" card model strips HTML formatting. Use `--card_model "Basic"` instead:
```bash
python -m generator.read-generate-import ./input.csv ./processing --card_model "Basic"
```
*****
Q: On macOS, images are not found in my cards.
A: Make sure to specify the correct Anki media directory path:
```bash
--anki_media_directory_path "$HOME/Library/Application Support/Anki2/User 1/collection.media"
```
